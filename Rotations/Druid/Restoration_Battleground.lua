...
ProbablyEngine.rotation.register_custom(105, "|cff00FFFFDr. Green Thumb|r - |cffFF7D0ABattleground v0.1|r", {
...
{{ -- Shapeshift: Cat Form
	...
	{ "!768", "player.state.root" },
	-- Maybe if target low auto mass entanglement?
	{ "108294", {"talent(6,1)","modifier.rshift"}}, -- Heart of the Wild
	...
	{ "106830", {"!target.debuff(106830)", "player.energy >= 50","target.range <= 5" },"target"}, -- Trash
...
{{ -- No Shapeshift
	...
	{{ -- Mouseover Dotting start
		{ "8921", "!mouseover.debuff(164812)", "mouseover"}, -- Moonfire
	}, "toggle.dotEverything" }, -- Mouseover dotting end
	
	{{ -- Damage no shapeshift Start
		{ "8921", "!target.debuff(164812)", "target"},-- Moonfire keep debuff up
		{ "5176", "!player.moving", "target"}, -- Wrath
		{ "8921", "player.moving", "target"}, -- Moonfire if you move
	},"modifier.lcontrol"}, -- Damage no shapeshift end

	---------------------------
	--    MODIFIERS/MISC     --
	---------------------------
	{"33786", "modifier.rshift", "target"},
	{"!102401", "modifier.lalt", "focus"}, --Rebirth
	{ "102793", "modifier.rcontrol", "player.ground"},
--{{ Not longer needed
--	{"/cancelform",{"player.buff(Stampeding Roar)"},},
--	{"/cancelform",{"player.buff(Displacer Beast)"},},},
-- "!player.buff(33891)"}, 137452
--{ "783", "player.state.root" }, -- Anti Root Shapeshift
---------------------------
--      SURVIVAL        --
---------------------------
{ "5211", "modifier.interrupts", "target" },
{"Ironbark",{"focus.health <= 40","focus.friend","focus.range <= 40"},"focus"},
{"Ironbark",{"tank.health <= 40","tank.range <= 40"},"tank"},
{ "88423", {"party2.range <= 40","party2.state.root",}, "party2" }, -- germ
...
}, {"!player.buff(768)", "!player.buff(5487)"}}, -- End Healing
}, -- End of Combat

{ -- Start Out of Combat
	{ "1126", "!player.buffs.stats" }, --stats
	-- {"Wild Mushroom", "modifier.lcontrol", "mouseover.ground" }, --Shroom
	{ "102280", {"talent(1,2)","modifier.lalt"}}, -- Displacer Beast
	{ "102280", {"talent(1,3)","modifier.lalt"}}, -- Displacer Beast
	{"!102401", "modifier.ralt", "focus"}, --Rebirth
	{{
	{ "774", "!player.buff(774)" , "player"},
	{ "!768",}, -- Go Cat
	}, "modifier.lshift"},
...
ProbablyEngine.toggle.create('dotEverything', 'Interface\\Icons\\Ability_creature_cursed_05.png', 'Dot All The Things!', 'Click here to dot all the things!\nSome Spells require Multitarget enabled also.\nOnly Works if using FireHack.')
